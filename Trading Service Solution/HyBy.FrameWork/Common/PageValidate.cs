...
namespace HyBy.FrameWork.Common
{
	...
	public class PageValidate
	{
        ...
        private static Regex RegCardID = new Regex("^(^\\d{15}$|^\\d{18}$|^\\d{17}(\\d|X|x))$");//身份证
        private static Regex RegMobilePhone = new Regex("^1\\d{10}$");//手机
        private static Regex RegTelePhone = new Regex("^((0\\d{2,3})-)(\\d{7,8})(-(\\d{3,}))?$");//座机
