...
{
    public class ExCommonServiceRs : BusinessObject, IDbExecuter
    {
        Random rd;
        /// <summary>
        /// 如果未传入数据连接及事务控制对象，则使用默认的
        /// </summary>
        public ExCommonServiceRs()
        {
            dac = new DataAccess("RsConnString");
            rd = new Random();
        }
        public ExCommonServiceRs(DataAccess dac)
        {
            if ((dac == null))
            {
                this.dac = new DataAccess("RsConnString");
            }
            else
            {
                this.dac = dac;
